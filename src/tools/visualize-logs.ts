...
const DATA_DIR = process.env.WCC_DATA_DIR || join(homedir(), '.wechat-codex');
...
function reconstructSessions(lines: ParsedLine[]): Session[] {
  ...
  for (const line of lines) {
    ...
    // Query completed
    if (line.message === 'Codex' && line.raw?.includes('Codex CLI query completed')) {
      if (current) {
    ...
    // Query aborted
    if (line.message === 'Codex' && line.raw?.includes('Codex CLI query aborted')) {
      if (current) {
        current.endTime = line.timestamp;
        const start = new Date(current.startTime).getTime();
        const end = new Date(line.timestamp).getTime();
    ...
    // Query timed out
    if (line.message === 'Codex' && line.raw?.includes('Codex query timed out')) {
      if (current) {
        current.hasError = true;
      }
      continue;
