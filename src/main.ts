...
import { codexQuery, type QueryOptions } from './claude/provider.js';
...
// Extensions eligible for auto-push when detected in Codex response
...
/** Extract local file paths from Codex response text. */
...
async function sendToCodex(
  ...
  try {
    ...
    // Auto-push deliverable files mentioned in Codex response
    if (result.text) {
