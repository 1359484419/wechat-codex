...
macos_start() {
  ...
  for var in CODEX_HOME CODEX_BIN OPENAI_API_KEY OPENAI_BASE_URL; do
    if [ -n "${!var:-}" ]; then
...
<dict>
  ...
  <dict>
    <key>PATH</key>
    <string>${HOME}/.local/bin:${node_bin%/*}:/Applications/Codex.app/Contents/Resources:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin</string>
...
Documentation=https://github.com/Wechat-ggGitHub/wechat-codex
...
Restart=always
RestartSec=10
Environment=PATH=${HOME}/.local/bin:${node_bin%/*}:/Applications/Codex.app/Contents/Resources:/usr/local/bin:/usr/bin:/bin
